...
% Notes: introducing failure probability AND variation in event amplitudes
% Two more parameters:
% evoked_params.sigma_a, evoked_params.failure_prob
% rng(12341)
...
% For each fired events:
    for i = 1:length(evoked_params.times)        
        
        tmpi = evoked_params.times(i); 
        check_temp =rand(1);
        % Add this event to the evoked train if it passes the failure check
        if  check_temp < evoked_params.failure_prob
        ...
        logC_ = 0;
        for ti = 1:nc
            a_init = normrnd(evoked_params.a(i),evoked_params.sigma_a) ;
%             tmpi_ = tmpi+(st_std*randn);
            tmpi_ = tmpi;
