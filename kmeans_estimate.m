...
%     k
    if do_plot && k > 0 && length(posterior.amp) >= k    
        ...
        plot(1:2000,traces(i,:) - traces(i,end) - 200 - 100*(i-1),'color',colors_lines(i,:))
        plot(1:2000, -1.0*build_curve(event_feature_means,0,.1,1/20000,2000) - 200 - 100*(i-1),'color',colors_lines(i,:),'Linewidth',2)
        xlim([1 2000])
    elseif do_plot
        plot(1:2000,traces(i,:) - traces(i,end) - 200 - 100*(i-1),'color',colors_lines(i,:))
        xlim([1 2000])
